...
<%@ page import="model.SolicitudesVO" %>
<%@ page import="java.util.ArrayList"%>
<%@ page import="java.time.LocalDate"%>

<%	Object error = request.getParameter("error");
	String msgErrorEspecial = "<br>";
	if (error != null) {
		if (error.toString().equals("especial")) msgErrorEspecial = "No existe ningún usuario especial con ese nombre"; 
} %>
...
<% UsuariosVO usuario = (UsuariosVO)request.getSession().getAttribute("usuario"); 
   ...
   ArrayList<SolicitudesVO> especiales = (ArrayList<SolicitudesVO>)request.getSession().getAttribute("especiales"); %>
	...
	<div class="row my-4 mx-2 justify-content-center">
	    ...
	    <div class="col-6">
	        <div class="card shadow px-2 mb-4">
               	<h5 class="mx-5 mt-4"> Gestionar solicitudes de cuentas especiales </h5>
                <hr>
                <div class="px-5">
					<!-- Dar privilegios o descartar solicitudes -->
                    <p> Decide a que usuarios aportar permisos especiales: </p>
                    <div class="form-control rounded-3 pt-3" style="height: 20rem; overflow:auto;">
                       	<% if (especiales.size() > 0)  {
                       		for (SolicitudesVO e:especiales) { %>
                       			<div class="row" id="<%= e.getNombre() %>">
                       				<div class="col">
                       					<h6> <%= e.getNombre() %> </h6>
                       				</div>
                       				<div class="col">
                       					<a href="TratarEspecial?tipo=aprobar&idEspecial=<%= e.getNombre() %>" class="btn btn-ap rounded-5"> Aprobar </a>
                       				</div>
                       				<div class="col">
                       					<a href="TratarEspecial?tipo=descartar&idEspecial=<%= e.getNombre() %>" class="btn btn-com rounded-5"> Decartar </a>
                       				</div>
                       			</div>
                       			<% if (! especiales.get(especiales.size()-1).getNombre().equals(e.getNombre())) { %>
				        			<hr class="mx-3">
				        		<% } %>
                       		<% }
                       	   } else { %>
                       		<p><i> No hay solicitudes que gestionar </i></p>
                       	<% } %>
                	</div>
					<!-- Quitar privilegios -->
                	<div class="my-4">
                		<p> Decide a que usuarios quitar permisos especiales: </p>
                		<form action="QuitarEspeciales" method="post">
                			<div class="row">
		                    	<div class="col-9">
		                    		<input required type="text" class="form-control rounded-3" name="userEspToNorm" placeholder="Escribe el nombre de un usuario...">
		                    	</div>
		                    	<div class="col">
		                    		<input class="btn btn-ap rounded-5" required type="submit" value="Quitar">
		                    	</div>
		                    </div>
