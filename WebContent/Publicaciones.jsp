...
<%@ page import="model.PublicacionesVO"%>
...
<% String start = request.getParameter("start");
   ...
   ArrayList<PublicacionesVO> publicaciones = (ArrayList<PublicacionesVO>)request.getSession().getAttribute("publicaciones");
%>
	...
	<div class="row my-4 mx-2 justify-content-center">
	    ...
	    <div class="col-6">
	            ...
	            <div class="card-body" style="display:inline-block">
	                <div class="row text-center" >
	                	<form class="col" action="MostrarPublicaciones" method="post">
	                    	<input class="btn btn-sel rounded-5 mx-2 my-1" required type="submit" value="Publicaciones">
	                    </form>
	                    <form class="col" action="IniciarEventos" method="post">
	                    	<input class="btn btn-no-sel rounded-5 mx-2 my-1" required type="submit" value="Eventos">
	                    </form>
	                </div>
	            </div>
	        </div>
	        ...
	        <div class="card shadow px-4 mb-4">
	            <div class="card-body">
                	<form action="SubirPublicacion" method="post">
	                	<div class="row d-flex align-items-center">
	                		<div class="col-9 mx-3">
	                			<input class="form-control rounded-3" required type="text" name="descripcion" placeholder="Comparte tus pensamientos con otros amantes de la música...">
	                		</div>
	                		<div class="col mx-3">
	                			<input class="btn btn-peq-s rounded-5 my-1" required type="submit" value="Publicar">
	                		</div>
	                	</div>
                    </form>
	            </div>
	        ...
	        <div class="card shadow px-2 my-4">
	        	<% int count=0; 
	        	   if (publicaciones.size() > 0) { %>
		        	<% for (PublicacionesVO p:publicaciones) { 
		        	       count++; %>
			        	<div id="<%= p.getId() %>" class="mx-5 my-4">
			        		<h4> <%= p.getNombre() %> </h4>	
			        		<p> <%= p.getDescripcion() %> </p>
			        		<a href="MostrarComentariosP?publicacionActual=<%=p.getId()%>" class="btn btn-com rounded-5"> Comentarios </a>
			        	</div>
				        <% if (publicaciones.get(publicaciones.size()-1).getId() != p.getId()) { %>
				        	<hr class="mx-3">
				        <% } %>
		        	<% } %>
		        <% } else { %>
		        	<div class="mx-5 mt-4 text-center">
		        		<p><i> No hay publicaciones que mostrar... <br> ¡Anímate y publica algo! </i></p>
		        	</div>
		        <% } %>
	        </div>
	        ...
	        <div class="row my-4 justify-content-center">
	        	<div class="col">
	        		<div class="card shadow px-2 mb-4">
	        			<div style="text-align:center" class="card-body">
	        				<a <% if (startAsInt>1) { %> href="MostrarPublicaciones?start=<%=startAsInt-endAsInt%>&end=<%=endAsInt%>" <% } %> > 
	        	...
	        	<div class="col">
	        		<div class="card shadow px-2 mb-4">
	        			<div style="text-align:center" class="card-body">
	        				<a <% if (endAsInt==count) { %> href="MostrarPublicaciones?start=<%=startAsInt+endAsInt%>&end=<%=endAsInt%>" <% } %> > 
